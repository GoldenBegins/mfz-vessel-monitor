...
\n// INTERPORT_DASHBOARD_V1\nfunction interportEsc(v){return String(v??'').replace(/[&<>"']/g,c=>({'&':'&amp;','<':'&lt;','>':'&gt;','"':'&quot;',"'":'&#39;'}[c]))}\nfunction interportDate(v){if(!v)return '—';if(typeof anchorageDate==='function')return anchorageDate(v);const s=String(v),m=s.match(/^(\\d{4})-(\\d{2})-(\\d{2})(?:[ T](\\d{1,2}):(\\d{2}))?/);if(!m)return s;return `${m[3]}-${m[2]}-${m[1]}${m[4]?` · ${String(m[4]).padStart(2,'0')}:${m[5]}`:''}`}\nfunction interportHours(v){const n=Number(v);if(!Number.isFinite(n))return '—';return n<24?`${n.toFixed(n<10?1:0)} س`:`${(n/24).toFixed(1)} يوم`}\nfunction renderInterport(d){const box=d&&d.interPortVoyages?d.interPortVoyages:{},s=box.summary||{},rows=Array.isArray(box.rows)?box.rows:[];const set=(id,val)=>{const el=document.getElementById(id);if(el)el.textContent=val};set('m2bConfirmed',s.misurataToBenghaziConfirmed||0);set('b2mConfirmed',s.benghaziToMisurataConfirmed||0);set('interportConfirmedTotal',s.confirmedTotal||0);set('interportCandidateTotal',s.candidateTotal||0);set('m2bCandidates',`${s.misurataToBenghaziCandidates||0} مرشح`);set('b2mCandidates',`${s.benghaziToMisurataCandidates||0} مرشح`);set('interportUpdated',box.updatedAt?interportDate(box.updatedAt):'—');const tbody=document.getElementById('interportRows');if(!tbody)return;if(!rows.length){tbody.innerHTML='<tr><td colspan="8" class="muted">لا توجد رحلات مؤكدة أو مرشحة بعد.</td></tr>';return}tbody.innerHTML=rows.map(r=>{const candidate=r.kind==='candidate';const direction=`${r.originPort==='Misurata'?'مصراتة':'بنغازي'} → ${r.destinationPort==='Misurata'?'مصراتة':'بنغازي'}`;const second=candidate?(r.destinationAnchorageFirstSeen||''):(r.destinationFirstSeen||'');const gap=Number.isFinite(Number(r.seaTransitCalendarGapDays))?`${Number(r.seaTransitCalendarGapDays)} يوم`:'—';const anchor=interportHours(r.destinationAnchorageHours);const stay=interportHours(candidate?r.originStayHours:r.destinationStayHours);const status=candidate?'<span class="voyage-status candidate">مرشح · في المخطاف</span>':'<span class="voyage-status confirmed">رحلة مؤكدة</span>';return `<tr><td><strong>${interportEsc(r.vessel||'—')}</strong><br><small>IMO ${interportEsc(r.imo||'—')}</small></td><td class="voyage-direction">${interportEsc(direction)}</td><td>${status}</td><td>${interportEsc(interportDate(r.originLastSeen))}</td><td>${interportEsc(interportDate(second))}</td><td>${interportEsc(gap)}</td><td>${interportEsc(anchor)}</td><td>${interportEsc(stay)}</td></tr>`}).join('')}\n
